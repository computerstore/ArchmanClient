...
public class CompanyEditor extends ScrolledCRMFormEditor {

	public static String ID = "it.archiworld.archman.client.crm.companyeditor"; //$NON-NLS-1$
	...
	public void doSave(IProgressMonitor monitor) {
		...
		try {
			...
			if((error=controlTaxCode(company.getTax_code()))!=""){ //$NON-NLS-1$
				MessageDialog.openError(this.getSite().getShell(),
						Messages.PersonEditor_SaveFailureDialog_Heading,
						error);
				return;
			}
			CompanyEditorInput input = (CompanyEditorInput) this
					.getEditorInput();
			company=(Company)input.createOrUpdate();
			MessageDialog.openInformation(this.getSite().getShell(),
					Messages.CompanyEditor_SaveSuccessDialog_Heading,
					Messages.CompanyEditor_SaveSuccessDialog_Text);
			setDirty(false);
			...
			MessageDialog.openError(this.getSite().getShell(),
					Messages.CompanyEditor_SaveFailureDialog_Heading,
					Messages.CompanyEditor_SaveFailureDialog_Text);
			t.printStackTrace();
		}
	}
	...
	protected void createFormBody(Composite body) {
		...
		createLabeledText(Messages.CompanyEditor_Label_TaxCode, company,
				"tax_code"); //$NON-NLS-1$
		createLabeledText(Messages.CompanyEditor_Fiscal_Code, company,
