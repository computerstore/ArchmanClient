...
public class SaveAsServiceMemberAction extends Action implements
	...
	public SaveAsServiceMemberAction(IWorkbenchWindow window) {
		this.window = window;
		icon = ImageDescriptor.createFromFile(this.getClass(),
				"/help/icons/wrench.png"); //$NON-NLS-1$
		setId(ID);
		setText(Messages.SaveAsServiceMemberAction_SaveAsServiceMemberActionText);
		setToolTipText(Messages.SaveAsServiceMemberAction_SaveAsServiceMemberActionToolTip);
	...
	public void run() {
		if (personEditor != null) {
			personEditor.doSaveAsServiceMember(null);
		} else {
			System.out
